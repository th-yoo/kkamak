...
kkamak also ships a config-gated extension seam, off by default, and one instrument built on it — `gauge` — documented separately in [`docs/gauge.md`](docs/gauge.md).
...
bun ~/.claude/plugins/cache/kkamak/kkamak/0.8.0/src/cli/init-cli.ts --check 'bun test'
# or from a checkout
...
kkamak ships a config-gated extension seam (`src/extensions/`), off by default: nothing under it runs, and nothing under it is even loaded, unless `gate.json` names it. Enable one by adding an `extensions` block:

```json
{ "check": "bun test", "extensions": { "gauge": true } }
```

One extension ships today: **gauge**, a shadow instrument, documented fully in [`docs/gauge.md`](docs/gauge.md). The essentials:

- **Cost.** Enabling gauge means kkamak makes real model calls — via the `claude` CLI (`claude -p ...`, one call per task-shaped user prompt), default `claude-haiku-4-5`, overridable with the `KKAMAK_GAUGE_MODEL` environment variable. Requires `claude` on `PATH`; a missing binary fails that call open (no gauge data, session unaffected), same as every other kkamak failure mode. Capped at 30 calls per repo per day.
- **Kill switch.** Set `KKAMAK_GAUGE=off` in the environment to stop new gauge calls immediately — no `gate.json` edit needed, and it takes effect on the next prompt, same as any other config change here.
- **Artifacts.** Gauge writes to `.km/gauge/` (covered by the same `.km/` `.gitignore` line the core gate already adds) and, when a derivation is available at Stop, adds a `gauge` field to that cycle's sensor line.
- **Shadow-only guarantee.** Gauge never changes a `block`/`allow` decision. It runs after the kernel has already decided, only annotating the sensor line the kernel is about to write; any check it derives runs strictly behind a read-only guard and is never treated as gating.
